...
if ( !class_exists( 'avia_masonry' ) )
{
	class avia_masonry
	{
		...
		function html()
		{
			...
			$output .= "<div id='av-masonry-".self::$element."' class='child-av-masonry av-masonry noHover av-{$size}-size av-{$this->atts['gap']}-gap av-hover-overlay-{$this->atts['overlay_fx']} av-masonry-col-{$this->atts['columns']} av-caption-{$this->atts['caption_display']} av-caption-style-{$this->atts['caption_styling']} {$this->atts['container_class']}' {$style} >";
			...
			if(empty($params['custom_query']))
            {
				...
				else
				{
					...
					$query = array(	'orderby' 	=> $params['query_orderby'],
						            //'orderby' => 'meta_value_num', //dgamoni
                                	//'meta_key' => 'custom_order_type_snv_1', //dgamoni
									//'orderby' => 'menu_order', //dgamoni
