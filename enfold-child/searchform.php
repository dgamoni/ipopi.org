...
$search_params = apply_filters('avf_frontend_search_form_param', array(
	...
	'placeholder'  	=> __('Search IPOPI website','avia_framework'),
	'search_id'	   	=> 's',
...
<form action="<?php echo $search_params['form_action']; ?>" id="searchform" method="get" class="<?php echo $disable_ajax; ?>">
	<div>
		<input type="submit" value="<?php echo $icon; ?>" id="searchsubmit" class="ipopisubmit button <?php echo $class; ?>" />
		<input class="custom_placeholder" type="text" id="s" name="<?php echo $search_params['search_id']; ?>" value="<?php if(!empty($_GET['s'])) echo get_search_query(); ?>" placeholder='<?php echo $search_params['placeholder']; ?>' />
		<?php 
